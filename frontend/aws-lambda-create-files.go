...
func AwsLambdaCreateFiles(app *config.EimerKette, cho myq.MyQueue, chstatus myq.MyQueue) {
	// Setup BackChannel
	startBackChannel(app, &app.Config.Frontend.AwsLambdaCreateFiles.BackChannelQ, cho, chstatus)
	// Issue Command
	...
	sqsClient := sqs.NewFromConfig(app.Config.Frontend.AwsLambdaCreateFiles.CommandQ.Aws.Cfg)
	msg := models.CmdCreateFiles{
		Command: "CreateFiles",
		Payload: models.CreateFilesPayload{
