...
func PulumiMain() {
	...
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		// 	return err
		// }
		queues := map[string]QParams{}
		for _, qtype := range []string{"command", "status"} {
			qp := QParams{}
			qp.qName = fmt.Sprintf("Eimer-Kette-%s", qtype)
			qp.Q, err = sqs.NewQueue(ctx, qp.qName, &sqs.QueueArgs{
				Name: pulumi.StringPtr(qp.qName),
			})
			if err != nil {
				return err
			}

			qp.qaccessName = fmt.Sprintf("%s-qaccess.%s", qp.qName, qtype)

			qp.qaccess, err = iam.NewRole(ctx, qp.qaccessName, &iam.RoleArgs{
			...
			iam.NewRolePolicyAttachment(ctx, fmt.Sprintf("%s-lamda.%s", qp.qaccessName, qtype), &iam.RolePolicyAttachmentArgs{
				Role:      qp.qaccess,
		...
		}`, current.AccountId, qp.qName)
			policyName := fmt.Sprintf("%s-sqs.%s", qp.qName, qtype)
		...
		lambdaName := fmt.Sprintf("%s-lambda", qc.qName)
		...
		_, err = lambda.NewEventSourceMapping(ctx, lambdaName, &lambda.EventSourceMappingArgs{
			EventSourceArn: qc.Q.Arn,
			FunctionName:   lambdaFn.Arn,
		})
		if err != nil {
			return err
		}
		for k, pq := range queues {
			ctx.Export(fmt.Sprintf("queues[%s]", k), pq.Q.Url)
		}
